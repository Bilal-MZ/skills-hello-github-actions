...
class PilotageECSDeciles:
    ...
    def piloter(self, idx):
        ...
        if (idx == 0) or self.phase_charge_ballon or (T_ballon < self.T_ballon_seuil):
            ...
            # On sort de la phase de charge si le ballon a atteint la température seuil
            self.phase_charge_ballon = T_ballon < self.T_ballon_seuil
            return

        # === PILOTAGE EN FONCTION DE LA DEMANDE (hors recharge) ===
        # Dans tous les cas, m_ECS_dir et m_ECS_V3V sont initialisés si on vient de sortir de la phase de charge
        if idx == 1:  # Juste après l'initialisation
            entrees[idx]['m_ECS_dir'] = 0.5 * self.m_ECS_dir_init
            entrees[idx]['m_ECS_V3V'] = 0.5 * self.m_ECS_V3V_init
            ...
            # Ajustement selon la température de sortie ECS
            ...
            # On s'assure que les valeurs restent positives et bornées
...
# 3. Réglage sortie de l'échangeur ECS
        ...
        # 4. Réglage du primaire pour le chauffage seul (identique à avant)
