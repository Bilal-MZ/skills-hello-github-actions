...
# données temporelles
time_step = 120               # ne pas modifier le pas de temps ici: appeler la fonction "initialize"
...
n_pdt = 4
T_prim_in = pd.Series(T_prim_in).rolling(n_pdt).mean()
T_prim_in = T_prim_in.fillna(T_prim_in.loc[n_pdt-1])
T_prim_in = T_prim_in.values
...
class PilotageLoiDEauTemperature:
    ...
    def piloter(self, idx):
        ...
        entrees[idx]['m_ECS_sout'] = m_ECS_sout[idx] # m_sout_ref[idx] # 0.1 * (np.sin(idx/(len(self.T_ch_in_ref)) * 2 * np.pi) + 1) / 2 + 0.05 # 0.25
        ...
        if idx == 0:                            # pas d'historique de mesure des températures, donc supposition de débits 
            ...
            entrees[idx]['m_ECS_dir'] = 0.2
            entrees[idx]['m_ECS_V3V'] = 0
            ...
            # Vérifier que le ballon était en mode décharge
                if entrees[idx-1]['m_ECS_stock'] < 0:
                    ...
                    if  sorties[idx-1]['T_ECS_n'] < sorties[idx-1]['T_ECS_sec_out']:
                    # continue la décharge
                        entrees[idx]['m_ECS_sec'] = entrees[idx]['m_ECS_sout'] + entrees[idx]['m_ECS_bouc'] + abs(entrees[idx]['m_ECS_stock'])     
                        if sorties[idx-1]['T_ECS_n'] < 60:
                        # arreter la décharge et commencer à nouveau la charge du ballon    
                            entrees[idx]['m_ECS_stock'] = -min(abs(entrees[idx]['m_ECS_stock']), 0.1)
                            entrees[idx]['m_ECS_sec'] = entrees[idx]['m_ECS_sout'] + entrees[idx]['m_ECS_bouc'] - abs(entrees[idx]['m_ECS_stock'])
                        else:
                        # recommencer la charge
                            entrees[idx]['m_ECS_stock'] = abs(entrees[idx-1]['m_ECS_stock'])
                            entrees[idx]['m_ECS_sec'] = entrees[idx]['m_ECS_sout'] + entrees[idx]['m_ECS_bouc'] + abs(entrees[idx]['m_ECS_stock'])  
                    else:
                        # Augmenter m_ECS_sec (ouvrir davantage la vanne vers m_ECS_sec)
                        dm_sec = min(abs(entrees[idx-1]['m_ECS_sec']),self.dm)
                        entrees[idx]['m_ECS_sec'] = entrees[idx-1]['m_ECS_sec'] + dm_sec
                        entrees[idx]['m_ECS_stock'] = abs(entrees[idx-1]['m_ECS_stock']) - dm_sec
       
            if sorties[idx-1]['T_ECS_sec_out'] < self.T_out_ECS_sec_ref:
                # Ouvrir davantage la vanne vers m_ECS_V3V
                if sorties[idx-1]['T_sec_out'] < sorties[idx-1]['T_ECS_prim_out']:
                    dm_V3V = min(abs(entrees[idx-1]['m_ECS_V3V']),self.dm)
                    entrees[idx]['m_ECS_dir'] = entrees[idx-1]['m_ECS_dir'] - dm_V3V
                    entrees[idx]['m_ECS_V3V'] = entrees[idx-1]['m_ECS_V3V']+ dm_V3V
            else:
...
def posttraiter(plot=True):
    ...
    if plot:
        ...
        fig.tight_layout()
         # 1. T_prim_in et T_prim_out
        fig1, ax1 = plt.subplots(figsize=(16, 5))
        entrees.iloc[slice_][['T_prim_in (in)']].plot(ax=ax1, label='T_prim_in', color='b')
        sorties.iloc[slice_][['T_prim_out']].plot(ax=ax1, label='T_prim_out', color='r')
        ax1.set_title('Températures primaire (entrée et sortie)')
        ax1.set_ylabel('Température [°C]')
        ax1.legend()
        fig1.tight_layout()
        fig1.canvas.manager.set_window_title('T_prim_in et T_prim_out')

        # 2. T_sec_in et T_sec_out
        fig2, ax2 = plt.subplots(figsize=(16, 5))
        sorties.iloc[slice_][['T_sec_in']].plot(ax=ax2, label='T_sec_in', color='b')
        sorties.iloc[slice_][['T_sec_out']].plot(ax=ax2, label='T_sec_out', color='r')
        ax2.set_title('Températures secondaire (entrée et sortie)')
        ax2.set_ylabel('Température [°C]')
        ax2.legend()
        fig2.tight_layout()
        fig2.canvas.manager.set_window_title('T_sec_in et T_sec_out')

        # 3. T_ECS_prim_in et T_ECS_prim_out
        fig3, ax3 = plt.subplots(figsize=(16, 5))
        sorties.iloc[slice_][['T_ECS_prim_in']].plot(ax=ax3, label='T_ECS_prim_in', color='b')
        sorties.iloc[slice_][['T_ECS_prim_out']].plot(ax=ax3, label='T_ECS_prim_out', color='r')
        ax3.set_title('Températures primaire ECS (entrée et sortie)')
        ax3.set_ylabel('Température [°C]')
        ax3.legend()
        fig3.tight_layout()
        fig3.canvas.manager.set_window_title('T_ECS_prim_in et T_ECS_prim_out')

        # 4. T_ECS_1, T_ECS_n, T_ballon_out sur une seule figure
        fig4, ax4 = plt.subplots(figsize=(16, 5))
        sorties.iloc[slice_][['T_ECS_1']].plot(ax=ax4, label='T_ECS_1', color='b')
        sorties.iloc[slice_][['T_ECS_n']].plot(ax=ax4, label='T_ECS_n', color='g')
        sorties.iloc[slice_][['T_ballon_out']].plot(ax=ax4, label='T_ballon_out', color='r')
        ax4.set_title('Températures ballon ECS')
        ax4.set_ylabel('Température [°C]')
        ax4.legend()
        fig4.tight_layout()
        fig4.canvas.manager.set_window_title('T_ECS_1, T_ECS_n, T_ballon_out')
