...
# === 3. Fonction utilitaire pour créer une loi d'eau ===
def creer_loi_d_eau(*points: tuple[float, float]):
    xp = [p[0] for p in points]
    yp = [p[1] for p in points]
    assert np.all(np.diff(xp) > 0)
    func = lambda x : np.interp(x, xp, yp) 
    return np.vectorize(func)

# === 4. Initialisation des données ===
time_step = 15  # secondes
P_ch_ref = pd.read_csv('donnees/batiment_1/NO.N.AB.01.Gen.ReEx.001.001_2014_2023.csv').iloc[:, 1].values
m_ECS_sout_ref = pd.read_csv('donnees/batiment_1/NO.AB.ts60.cat4.occ34.persday40.res1.csv').iloc[:, 1].values
T_prim_out_ref = np.full(8760, 85)
T_ECS_in_ref = 60
T_out_ECS_ref = 53

df = pd.read_csv('donnees/batiment_1/TMY_Trondheim_2014_2023.csv')
T_ext =  df['temp_air'].values
I_dir =  df['dni'].values
I_diff = df['dhi'].values
T_prim_in = creer_loi_d_eau((-10, 140), (15, 100))(T_ext)
T_prim_in = pd.Series(T_prim_in).rolling(4).mean().fillna(180).values
m_prim = np.full(8760, 0.1)

# === 5. Rééchantillonnage/interpolation des séries temporelles ===
# (Garde la logique de ton code pour interpolation, découpage saison, time_share, etc.)

time_share = 0.005
for idx, var_name in enumerate(['P_ch_ref', 'm_ECS_sout_ref', 'T_prim_out_ref', 'T_ext', 'I_dir', 'I_diff', 'T_prim_in', 'm_prim']):
    var_ = globals()[var_name]
    time_step_old = 365 * 24 * 3600 / len(var_)
    var_ = np.append(var_, var_[0])
...
# === 6. Initialisation automatique des entrées ===
def initialiser_entrees(idx):
    for key in mapper_U.values():
        entrees[idx][key] = 0  # Ou une valeur par défaut intelligente
...
# === 7. Classes de pilotage ===

class PilotageChauffagePID:
    def __init__(self):
        self.T_air = getP_Radiateurs_T_air()
        self.T_ch_in_ref = creer_loi_d_eau((-15, 110), (-5, 85), (15, 30), (16, self.T_air))(T_ext)
        self.m_ch_ref = creer_loi_d_eau((-15, 0.175), (15, 0.175), (16, 0))(T_ext)
        self.dm = 0.0025
        self.integral = 0
        self.Kp = 0.10
        self.Ki = 0.01
        self.prev_T_ch_in_err = 0
    def piloter(self, idx):
        # Correction PID pour le chauffage
        if idx == 0:
            entrees[idx]['m_ch_dir'] = self.m_ch_ref[idx] / 2
            entrees[idx]['m_ch_V3V'] = self.m_ch_ref[idx] / 2
        else:
            T_ch_in = (entrees[idx-1]['m_ch_dir']*sorties[idx-1]['T_sec_out'] + entrees[idx-1]['m_ch_V3V']*sorties[idx-1]['T_ch_out'])/(self.m_ch_ref[idx-1] + 1e-6)
            erreur = self.T_ch_in_ref[idx] - T_ch_in
            self.integral += erreur
            delta_m = self.Kp * erreur + self.Ki * self.integral
            entrees[idx]['m_ch_dir'] = max(entrees[idx-1]['m_ch_dir'] + delta_m, 0)
            entrees[idx]['m_ch_V3V'] = max(entrees[idx-1]['m_ch_V3V'] - delta_m, 0)
            # Mise à l'échelle pour respecter le débit total
            total = entrees[idx]['m_ch_dir'] + entrees[idx]['m_ch_V3V']
            if total > 0:
                ratio = self.m_ch_ref[idx] / total
                entrees[idx]['m_ch_dir'] *= ratio
                entrees[idx]['m_ch_V3V'] *= ratio

class PilotageECSPrevision:
    def __init__(self, m_ECS_sout, horizon_prevision=3, T_ballon_seuil=65, m_ECS_stock_max=0.1):
        self.m_ECS_sout = m_ECS_sout
        self.horizon = horizon_prevision
        self.T_ballon_seuil = T_ballon_seuil
        self.m_ECS_stock_max = m_ECS_stock_max
        self.dm = 0.0025
    def prevoir_demande(self, idx):
        fin = min(len(self.m_ECS_sout), idx + self.horizon)
        return np.mean(self.m_ECS_sout[idx:fin])
    def piloter(self, idx):
        # Anticipation de demande ECS : on charge le ballon avant la pointe
        T_ballon = sorties[idx-1]['T_ballon_out'] if idx > 0 else self.T_ballon_seuil + 1
        future = self.prevoir_demande(idx)
        seuil = np.quantile(self.m_ECS_sout, 0.8)
        if (T_ballon < self.T_ballon_seuil) or (future > seuil):
            entrees[idx]['m_ECS_stock'] = -self.m_ECS_stock_max # recharge ballon
        else:
            entrees[idx]['m_ECS_stock'] = 0
        # Répartition des débits pour l'ECS :
        entrees[idx]['m_ECS_sout'] = self.m_ECS_sout[idx]
        entrees[idx]['m_ECS_dir'] = 0.5 * self.m_ECS_sout[idx]
        entrees[idx]['m_ECS_V3V'] = 0.5 * self.m_ECS_sout[idx]
        entrees[idx]['m_ECS_sec'] = 0.0
        entrees[idx]['m_ECS_bouc'] = 0.01 # bouclage minimum

# === 8. Fonctions d’exécution du modèle, d’évaluation et de post-traitement ===
...
def calculer_erreur_totale(idx):
    ...
    erreur1 = ((P_ch_modele - P_ch_ref[idx]) / (P_ch_ref[idx]+1)) ** 2
    ...
    erreurs[idx]['erreur3'] = erreur3
    erreurs[idx]['erreur4'] = erreur4
    erreurs[idx]['erreur'] =  erreur

def executer_modele_phy(idx):
    entrees_ = [entrees[idx][mapper_U[k]] for k in range(len(mapper_U))]
    setU(entrees_)
...
# === 9. Boucle principale de simulation ===

def piloter():
    lib.initialize(byref(c_int(time_step)))
    pilote_ch = PilotageChauffagePID()
    pilote_ecs = PilotageECSPrevision(m_ECS_sout_ref)
    for idx in tqdm.trange(nbr_ts):
        entrees[idx] = {}
        sorties[idx] = {}
        erreurs[idx] = {}
        initialiser_entrees(idx)
        # Ajout des consignes de base
        entrees[idx]['T_prim_in'] = T_prim_in[idx]
        entrees[idx]['T_edv'] = 10
        entrees[idx]['m_prim'] = m_prim[idx]
        # Pilotage ECS
        pilote_ecs.piloter(idx)
        # Pilotage chauffage
        pilote_ch.piloter(idx)
